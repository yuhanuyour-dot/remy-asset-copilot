...
Get-ChildItem -LiteralPath (Join-Path $root 'releases') -Filter 'RemyAssetCopilot-Setup-0.5.4-*.exe' | Where-Object { $_.Name -notlike '*-QA.exe' } | ForEach-Object {"$((Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash.ToLower())  $($_.Name)"} | Set-Content -LiteralPath (Join-Path $root 'releases/SHA256SUMS-0.5.4.txt') -Encoding ascii
